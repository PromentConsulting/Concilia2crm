...
<div
    x-data="{
        advancedOpen: false,
        fields: [
            { key: 'numero', label: 'Nº Factura', type: 'string' },
            { key: 'descripcion', label: 'Descripción', type: 'string' },
            { key: 'cliente', label: 'Cliente', type: 'string' },
            { key: 'forma_pago', label: 'Forma de pago', type: 'string' },
            { key: 'instruccion_pago', label: 'Instrucción de pago', type: 'string' },
            { key: 'dpto_comercial', label: 'Dpto. Comercial', type: 'string' },
            { key: 'email_facturacion', label: 'Email facturación', type: 'string' },
            { key: 'importe', label: 'Importe', type: 'number' },
            { key: 'importe_total', label: 'Importe total', type: 'number' },
            { key: 'fecha_factura', label: 'Fecha factura', type: 'date' },
            { key: 'fecha_vencimiento', label: 'Vencimiento', type: 'date' },
            { key: 'fecha_cobro', label: 'Fecha cobro', type: 'date' },
            { key: 'cobrado', label: 'Cobrado', type: 'string' },
            { key: 'contabilizado', label: 'Contabilizado', type: 'string' },
        ],
        stringOps: [
...
>
    <header class="flex flex-wrap items-center justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">Listado de facturas emitidas.</p>
        </div>
    ...
    <section class="rounded-2xl bg-white p-4 shadow-sm space-y-4">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <form method="GET" action="{{ route('facturas.index') }}" class="flex flex-1 flex-wrap items-center gap-3">
            <div class="relative flex-1 min-w-[220px]">
                ...
                <input
                    ...
                    name="q"
                    value="{{ $q }}"
                    placeholder="Buscar por nº factura, descripción o cliente..."
                    ...
                    <div class="relative z-50 w-full max-w-2xl rounded-2xl bg-white p-5 text-xs shadow-xl">
                        <div class="flex items-start justify-between gap-3 border-b border-slate-100 pb-3">
                            <div>
                                ...
                                <p class="mt-1 text-xs text-slate-500">Define condiciones avanzadas para filtrar facturas.</p>
                            </div>
                            <button
                                type="button"
                        ...
                        <form method="GET" action="{{ route('facturas.index') }}" class="space-y-3 pt-4">
                            <input type="hidden" name="q" value="{{ $q }}">
                            <input type="hidden" name="vista_id" value="{{ optional($activeView)->id }}">
                            @foreach ($activeColumnKeys as $key)
                                <input type="hidden" name="columns[]" value="{{ $key }}">
                            ...
                            <div class="mb-2 flex items-center gap-2">
                                <span class="mr-1 text-[11px] text-slate-500">Mostrar facturas que</span>
                                <button
                                    type="button"
            ...
            <div x-data="{ open: false }" class="relative">
                ...
                >
                    <form method="GET" action="{{ route('facturas.index') }}" class="space-y-2">
                        <input type="hidden" name="q" value="{{ $q }}">
                        <input type="hidden" name="vista_id" value="{{ optional($activeView)->id }}">
                        <input type="hidden" name="af" value="{{ $advanced }}">
                        ...
                        <tr class="hover:bg-slate-50">
                            @foreach ($activeColumnKeys as $colKey)
                                <td class="px-3 py-2 text-sm text-slate-700">
                                    @switch($colKey)
                                        @case('numero')
                                            <a href="{{ route('facturas.show', $factura) }}" class="font-medium text-slate-900 hover:text-[#9d1872] hover:underline">
                                        ...
                                        @case('descripcion')
                                            ...
                                            @break
                                        @case('importe')
                                            {{ $factura->importe !== null ? number_format($factura->importe, 2, ',', '.') . ' ' . $factura->moneda : '—' }}
                                            @break
                                        @case('importe_total')
                                        ...
                                        @case('fecha_cobro')
                                            ...
                                            @break
                                        @case('cobrado')
                                            <span class="inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold {{ $factura->cobrado ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600' }}">
                                                {{ $factura->cobrado ? 'Sí' : 'No' }}
                                            </span>
                                            @break
                                        @case('contabilizado')
                                            <span class="inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold {{ $factura->contabilizado ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600' }}">
                            ...
                            <td class="px-3 py-2 text-right text-xs">
                                <a href="{{ route('facturas.show', $factura) }}" class="text-[#9d1872] hover:underline">
                                    Ver
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="{{ count($activeColumnKeys) + 1 }}" class="px-3 py-6 text-center text-xs text-slate-500">
                                No se han encontrado facturas con los filtros aplicados.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            ...
            >
                ...
                <div class="relative z-50 w-full max-w-md rounded-2xl bg-white p-5 shadow-xl">
                    <h2 class="text-sm font-semibold text-slate-900">
                        Guardar vista de facturas
                    </h2>
                    <p class="mt-1 text-xs text-slate-500">
                        Guarda esta combinación de filtros y columnas como una vista reutilizable.
                    </p>

                    <form method="POST" action="{{ route('facturas.views.store') }}" class="mt-4 space-y-4">
                        @csrf
