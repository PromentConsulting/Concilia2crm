@php
    // Inicializamos líneas para el front
    $init = old('lineas');

    if (!$init && isset($pedido) && $pedido) {
        $init = $pedido->lineas->map(function ($l) {
            return [
                ...
                'fecha_limite_factura'  => optional($l->fecha_limite_factura)->format('Y-m-d'),
            ];
        })->values()->toArray();
    }
    ...
    if (!$init) {
        $init = [[
            ...
            'cantidad' => 1, 'precio' => 0,
            'descuento_porcentaje' => 0, 'iva_porcentaje' => 21,
            'fecha_limite_factura' => null,
        ]];
    }
...
<section
    x-data="{
        ...
        add() {
            this.lineas.push({ service_id:null, referencia:'', descripcion:'', cantidad:1, precio:0, descuento_porcentaje:0, iva_porcentaje:21, fecha_limite_factura:null })
        },
        openServiciosModal() {
...
>
        ...
        <div class="w-full max-w-4xl rounded-2xl bg-white shadow-xl">
            <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                <div>
                    ...
                    <p class="text-sm text-slate-500">Selecciona uno o varios servicios para incorporarlos al pedido.</p>
                </div>
        ...
        <table class="min-w-full text-sm">
            <thead class="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                <tr>
                    ...
                    <th class="px-3 py-2 text-left">F. límite factura</th>
            ...
            <tbody class="divide-y divide-slate-100" >
                    ...
                    <tr class="bg-white">
                        ...
                        <td class="px-3 py-2">
                            <input type="text" class="w-full rounded border border-slate-200 px-2 py-1 text-sm"
                                   x-model="l.descripcion" :name="`lineas[${i}][descripcion]`">
                        </td>
                        ...
                        <td class="px-3 py-2">
                            <input type="date" class="rounded border border-slate-200 px-2 py-1 text-sm"
                                   x-model="l.fecha_limite_factura" :name="`lineas[${i}][fecha_limite_factura]`">
                        </td>
                ...
                <tr class="bg-slate-50 font-medium">
                    <td colspan="7" class="px-3 py-3 text-right">Total pedido</td>
                    <td colspan="2" class="px-3 py-3 text-right tabular-nums" x-text="totalPedido().toFixed(2)"></td>
                    <td></td>
                </tr>
            </tbody>
        </table>
    ...
    {{-- Pasamos el total al backend (por si lo quieres usar). El backend recalcula igualmente. --}}
    <input type="hidden" name="importe_total" :value="totalPedido().toFixed(2)">
</section>
